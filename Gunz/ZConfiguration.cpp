...
bool ZConfiguration::LoadConfig(const char* szFileName)
{
	...
	if (!parentElement.IsEmpty())
	{
		...
		if (parentElement.FindChildNode(ZTOK_ETC, &childElement))
		{
			...
			childElement.GetChildContents(&m_Etc.bfps, ZTOK_ETC_FPS);	// Edit ShowFPS
			childElement.GetChildContents(&m_Etc.bhpap, ZTOK_ETC_HPAP);	// Edit ShowHPAP
...
//	if( parentElement.FindChildNode(ZTOK_MACRO, &childElement) )
	...
	// Etc
	{
		...
		//ShowFPS
		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_ETC_FPS);
		sprintf(temp, "%s", m_Etc.bfps?"TRUE":"FALSE");
		aElement.SetContents(temp);
		
		//ShowHPAP
		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_ETC_HPAP);
...
void ZConfiguration::Init()
{
	...
	m_Etc.bfps = true;	// Edit ShowFPS
	m_Etc.bhpap = true;	// Edit ShowHPAP
