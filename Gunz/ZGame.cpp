...
{
sprintf(szName, "!^9HP: [%d] - AP: [%d]",(int)ZGetGame()->m_pMyCharacter->GetHP(),(int)ZGetGame()->m_pMyCharacter->GetAP());
...
//MY FPS
sprintf(szName, "^9My FPS : [%3.3f]",g_fFPS,fScore,fMs);
ZApplication::GetGameInterface()->GetChat()->Input(szName);
}																				
////////테스트 용 코드...나중에 지울것..//////////////////////////////
////////////////////////////////////////////////////////////////////for test
...
// Edit thenm vip va quan tri
bool ZGame::GetUserGradeIDColor(MMatchUserGradeID gid,MCOLOR& UserNameColor,char* sp_name)
{
	int i = 0;
	if(gid == MMUG_ADMIN) 
	{
		int nCount = ( timeGetTime() / 999)% 10; // To change follow this:  999 = time  / 10 = how many colors
		for (i; i < nCount;  i++); //abre ai o ZcombatInterface.cpp

		if(i == 0)
		UserNameColor = MCOLOR(255,0,0);			// Red	
		if(i == 1)
		UserNameColor = MCOLOR(0,255,255);          // Cyan				
		if(i == 2)
		UserNameColor = MCOLOR(224,160,60);			// Orange
		if(i == 3)
		UserNameColor = MCOLOR(255,255,0);			// Yellow		
		if(i == 4)
		UserNameColor = MCOLOR(0,255,0);			// Green			
		if(i == 5)
		UserNameColor = MCOLOR(0,120,255);			// Dark Blue			
		if(i == 6)
		UserNameColor = MCOLOR(178,103,236);		// Purple		
		if(i == 7)
		UserNameColor = MCOLOR(213,255,0);			// Lime				
		if(i == 8)
		UserNameColor = MCOLOR(104,200,255);		// Light Blue		
		if(i == 9)
		UserNameColor = MCOLOR(255,0,255);			// Pink
		return true;  
	}
	else if(gid == MMUG_ROOT) 
	{
		int nCount = ( timeGetTime() / 999)% 10; // To change follow this:  999 = time  / 10 = how many colors
		for (i; i < nCount;  i++); //abre ai o ZcombatInterface.cpp
	...
	{ 
		UserNameColor = MCOLOR(204,0,0); 
		return true; 
	}
	else if(gid == MMUG_OP) 
	{
		UserNameColor = MCOLOR(0,204,255); 
		return true; 		
	}
	...
	{
		UserNameColor = MCOLOR(0,122,0); 
		return true; 
	}
	else if(gid == MMUG_VIP1) 
	{
		UserNameColor = MCOLOR(255,128,0); 
		return true; 
	}
	else if(gid == MMUG_VIP2) 
	{
		UserNameColor = MCOLOR(255,255,0); 
		return true; 
	}
	else if(gid == MMUG_VIP3) 
	{
		UserNameColor = MCOLOR(255,0,128); 
		return true; 
	}
	else if(gid == MMUG_VIP4) 
	{
		UserNameColor = MCOLOR(255,0,255); 
		return true; 
	}
	else if(gid == MMUG_VIP5) 
	{
		UserNameColor = MCOLOR(128,0,255); 
		return true; 
	}
	else if(gid == MMUG_VIP6) 
	{
		UserNameColor = MCOLOR(72,209,204); 
		return true; 
	}
	else if(gid == MMUG_VIP7) 
	{
		UserNameColor = MCOLOR(0,128,255); 
		return true; 
	}
	else if(gid == MMUG_VIP8) 
	{
		UserNameColor = MCOLOR(0,0,255); 
		return true; 
	}
	else if(gid == MMUG_VIP9) 
	{
		UserNameColor = MCOLOR(134,203,44); 
		return true; 
	}
	else if(gid == MMUG_VIP10) 
	{
		UserNameColor = MCOLOR(18,226,157); 
		return true; 
	}
	else if(gid == MMUG_VIP11) 
	{
		UserNameColor = MCOLOR(46,197,110); 
		return true; 
	}
	else if(gid == MMUG_VIP12) 
	{
		UserNameColor = MCOLOR(92, 30, 8); 
		return true; 
	}
	else if(gid == MMUG_VIP13) 
	{
		UserNameColor = MCOLOR(255,255,255); 
		return true; 
	}
	else if(gid == MMUG_VIP14) 
	{
		UserNameColor = MCOLOR(0,0,0); 
		return true; 
	}	
	else if(gid == MMUG_VIP15) 
	{
		...
		UserNameColor = MCOLOR(255,128,0); 
		if(i == 1)
		UserNameColor = MCOLOR(144,0,255); 			
		if(i == 2)
		UserNameColor = MCOLOR(255,128,0); 
		if(i == 3)
		UserNameColor = MCOLOR(255,255,0); 	
		...
		UserNameColor = MCOLOR(255,0,128); 		
		if(i == 5)
		UserNameColor = MCOLOR(255,128,128); 		
		if(i == 6)
		UserNameColor = MCOLOR(72,209,204); 
		if(i == 7)
		UserNameColor = MCOLOR(255,0,255); 			
		if(i == 8)
		UserNameColor = MCOLOR(0,255,128); 
		if(i == 9)
		UserNameColor = MCOLOR(128,0,255); 
		if(i == 10)
		UserNameColor = MCOLOR(0,0,255); 
...
//			gid = MMUG_DEVELOPER;//test
	...
	return ZGame::GetUserGradeIDColor( gid, UserNameColor, sp_name );
}
...
{
	// Edit co the cuoi khi ket thuc round
