...
void ZCombatInterface::OnDraw(MDrawContext* pDC)
{
	...
	// Edit tag [name]
	if(strstr(strlwr((char*)ZGetGameClient()->GetStageName()), "[name]"))
	{
	...
	if(IsShowUI())				// 모든 UI 감추기... by kam 20081020
	{
		...
		if (!m_bSkipUIDrawByRule)
		{
			...
			DrawHPAPNumbers(pDC);	// Edit ShowFPS ShowHPAP
...
void ZCombatInterface::DrawMyWeaponPont(MDrawContext* pDC)
{
	...
	// Edit vi tri ten vu khi
	TextRelative(pDC, 710.f/800.f, 510.f/600.f,m_szItemName);

	// Edit thong tin vu khi
	...
	// 705.f (ngang) / 800.f, 500.f (doc) / 600.f
	TextRelative(pDC, 690.f / 800.f, 520.f / 600.f, szWeaponInfo);
	}
	// 탄알수
...
void ZCombatInterface::SetPickTarget(bool bPick, ZCharacter* pCharacter)
{
	...
	if (bPick)
	{
		...
		m_pTargetLabel->SetTextColor(0xFFFF0080);			// Orange
		if(i == 1)
		m_pTargetLabel->SetTextColor(0xFF0080FF);			// Purple
		if(i == 2)
		m_pTargetLabel->SetTextColor(0xFFFF8000);			// Red	
		if(i == 3)
		m_pTargetLabel->SetTextColor(0xFFFF00FF);			// Yellow
		if(i == 4)
		m_pTargetLabel->SetTextColor(0xFF12E29D);			// Pink			
		if(i == 5)
		m_pTargetLabel->SetTextColor(0xFF48d1cc);			// Dark Blue
		if(i == 6)
		m_pTargetLabel->SetTextColor(0xFFFFFF00);			// Cyan			
		if(i == 7)
		m_pTargetLabel->SetTextColor(0xFF0000FF);			// Lime
		if(i == 8)
		m_pTargetLabel->SetTextColor(0xFF86CB2C);			// Light Blue
		if(i == 9)
		m_pTargetLabel->SetTextColor(0xFF8000FF);			// Green
		if(i == 10)
		m_pTargetLabel->SetTextColor(0xFF2EC56E);			// Pink
		if(i == 11)
		m_pTargetLabel->SetTextColor(0xFF5C1E08);			// Pink
		if(i == 12)
		m_pTargetLabel->SetTextColor(0xFFFFFFFF);			// Pink
		if(i == 13)
		m_pTargetLabel->SetTextColor(0xFF000000);			// Pink
		}
		if(pCharacter->IsRootName())
		{
		int i = 0;
		int nCount = ( timeGetTime() / 500)% 14; // To change follow this:  999 = time  / 10 = how many colors
		for ( i; i < nCount;  i++);
		...
		if(pCharacter->IsOpName())
			m_pTargetLabel->SetTextColor(ZCOLOR_OP_NAME);
		...
		if(pCharacter->IsVIP8Name())
			m_pTargetLabel->SetTextColor(ZCOLOR_VIP8_NAME);
		if(pCharacter->IsVIP9Name())
			m_pTargetLabel->SetTextColor(ZCOLOR_VIP9_NAME);
		if(pCharacter->IsVIP10Name())
			m_pTargetLabel->SetTextColor(ZCOLOR_VIP10_NAME);
		if(pCharacter->IsVIP11Name())
			m_pTargetLabel->SetTextColor(ZCOLOR_VIP11_NAME);	
		if(pCharacter->IsVIP12Name())
			m_pTargetLabel->SetTextColor(ZCOLOR_VIP12_NAME);
		if(pCharacter->IsVIP13Name())
			m_pTargetLabel->SetTextColor(ZCOLOR_VIP13_NAME);
		if(pCharacter->IsVIP14Name())
			m_pTargetLabel->SetTextColor(ZCOLOR_VIP14_NAME);
		if(pCharacter->IsVIP15Name())
		{
		int i = 0;
		int nCount = ( timeGetTime() / 500)% 14; // To change follow this:  999 = time  / 10 = how many colors
		for ( i; i < nCount;  i++);
				...
				if(pPickedCharacter->IsAdminName()) 
{
		...
		pDC->SetColor(0xFF000000);			// Pink
				...
				else if(pPickedCharacter->IsRootName()) 
	{
		pFont = MFontManager::Get("FONTa12_O1Org");
		int i = 0;
		int nCount = ( timeGetTime() / 500)% 14; // To change follow this:  999 = time  / 10 = how many colors
		...
		pDC->SetColor(0xFF000000);			// Pink
				...
				else if(pPickedCharacter->IsOpName()) {
					pFont = MFontManager::Get("FONTa12_O1Org");
					pDC->SetColor(MCOLOR(ZCOLOR_OP_NAME));
				}
				else if(pPickedCharacter->IsDeveloperName()) {
					pFont = MFontManager::Get("FONTa12_O1Org");
					pDC->SetColor(MCOLOR(ZCOLOR_DEVELOPER_NAME));
				}
				else if(pPickedCharacter->IsEventMasterName()) {
					pFont = MFontManager::Get("FONTa12_O1Org");
					pDC->SetColor(MCOLOR(ZCOLOR_EVENTMASTER_NAME));
				}
				else if(pPickedCharacter->IsMuteName()) {
					pFont = MFontManager::Get("FONTa12_O1Red");
					pDC->SetColor(MCOLOR(ZCOLOR_MUTE_NAME));
				}
				else if(pPickedCharacter->IsVipName()) {
					pFont = MFontManager::Get("FONTa12_O1Red");
					pDC->SetColor(MCOLOR(ZCOLOR_VIP_NAME));
				}
				...
				else if(pPickedCharacter->IsVIP8Name()) {
					pFont = MFontManager::Get("FONTa12_O1Red");
					pDC->SetColor(MCOLOR(ZCOLOR_VIP8_NAME));
				}
				else if(pPickedCharacter->IsVIP9Name()) {
					pFont = MFontManager::Get("FONTa12_O1Red");
					pDC->SetColor(MCOLOR(ZCOLOR_VIP9_NAME));
				}
				else if(pPickedCharacter->IsVIP10Name()) {
					pFont = MFontManager::Get("FONTa12_O1Red");
					pDC->SetColor(MCOLOR(ZCOLOR_VIP10_NAME));
				}
				else if(pPickedCharacter->IsVIP11Name()) {
					pFont = MFontManager::Get("FONTa12_O1Red");
					pDC->SetColor(MCOLOR(ZCOLOR_VIP11_NAME));
				}
				else if(pPickedCharacter->IsVIP12Name()) {
					pFont = MFontManager::Get("FONTa12_O1Red");
					pDC->SetColor(MCOLOR(ZCOLOR_VIP12_NAME));
				}
				else if(pPickedCharacter->IsVIP13Name()) {
					pFont = MFontManager::Get("FONTa12_O1Red");
					pDC->SetColor(MCOLOR(ZCOLOR_VIP13_NAME));
				}
				else if(pPickedCharacter->IsVIP14Name()) {
					pFont = MFontManager::Get("FONTa12_O1Red");
					pDC->SetColor(MCOLOR(ZCOLOR_VIP14_NAME));
				}
				else if(pPickedCharacter->IsVIP15Name()) 
	{
		pFont = MFontManager::Get("FONTa12_O1Red");
		int i = 0;
		int nCount = ( timeGetTime() / 500)% 14; // To change follow this:  999 = time  / 10 = how many colors
...
//	int nMaxLineCount=int((fHeight-fTitleHeight)*(float)RGetScreenHeight()/((float)pFont->GetHeight()*1.1f));
		...
		itor != ZGetCharacterManager()->end(); ++itor)
	{
		...
		if(pCharacter->IsAdminName())
		sprintf(pItem->szGrade,"%s","Administrator");
		else
		if(pCharacter->IsRootName())
		sprintf(pItem->szGrade,"%s","Root");
		else
		if(pCharacter->IsDeveloperName())
		sprintf(pItem->szGrade,"%s","Dev");
		else
		if(pCharacter->IsEventMasterName())
		sprintf(pItem->szGrade,"%s","Event Master");	
		...
		itor != ZGetCharacterManager()->end(); ++itor)
	{
		...
		if(pCharacter->IsAdminName()) { // << Change the Rank/Grade here
			sprintf(pItem->szLevel,"--Lv.",pCharacter->GetProperty()->nLevel, ZMsg(MSG_CHARINFO_LEVELMARKER));
//			pItem->SetColor(ZCOLOR_ADMIN_NAME);

					//vamos testar ve se nao tem erros ai.
		int nCount = ( timeGetTime() / 800)% 10; // To change follow this:  999 = time  / 10 = how many colors
		int i = 0;
		for (i; i < nCount;  i++);

		if(i == 0)                             		// To change the colors just change the 6 numbers after 0xff to your colors (hex)
		pItem->SetColor(0xffff0000);			// Red
		if(i == 1)
		pItem->SetColor(0xff00ffff);			// Cyan
		if(i == 2)
		pItem->SetColor(0xffe0a03c);			// Orange
		if(i == 3)
		pItem->SetColor(0xffffff00);			// Yellow
		if(i == 4)
		pItem->SetColor(0xff00ff00);			// Green
		if(i == 5)
		pItem->SetColor(0xff0078ff);			// Dark Blue
		if(i == 6)
		pItem->SetColor(0xffb267ec);			// Purple
		if(i == 7)
		pItem->SetColor(0xffd5ff00);			// Lime
		if(i == 8)
		pItem->SetColor(0xff68C8FF);			// Light Blue
		if(i == 9)
		pItem->SetColor(0xffff00ff);			// Pink
				}
		else if(pCharacter->IsRootName()) {
			sprintf(pItem->szLevel,"--Lv.",pCharacter->GetProperty()->nLevel, ZMsg(MSG_CHARINFO_LEVELMARKER));
//			pItem->SetColor(ZCOLOR_ADMIN_NAME);
		...
		pItem->SetColor(0xffff00ff);			// Pink
			...
			sprintf(pItem->szLevel,"%d%s",pCharacter->GetProperty()->nLevel, ZMsg(MSG_CHARINFO_LEVELMARKER));
			pItem->SetColor(ZCOLOR_VIP9_NAME);
		}
		else if(pCharacter->IsVIP10Name()) {
			sprintf(pItem->szLevel,"%d%s",pCharacter->GetProperty()->nLevel, ZMsg(MSG_CHARINFO_LEVELMARKER));
			pItem->SetColor(ZCOLOR_VIP10_NAME);
		}
		else if(pCharacter->IsVIP11Name()) {
			sprintf(pItem->szLevel,"%d%s",pCharacter->GetProperty()->nLevel, ZMsg(MSG_CHARINFO_LEVELMARKER));
			pItem->SetColor(ZCOLOR_VIP11_NAME);
		}	
		else if(pCharacter->IsVIP12Name()) {
			sprintf(pItem->szLevel,"%d%s",pCharacter->GetProperty()->nLevel, ZMsg(MSG_CHARINFO_LEVELMARKER));
			pItem->SetColor(ZCOLOR_VIP12_NAME);
		}
		else if(pCharacter->IsVIP13Name()) {
			sprintf(pItem->szLevel,"%d%s",pCharacter->GetProperty()->nLevel, ZMsg(MSG_CHARINFO_LEVELMARKER));
			pItem->SetColor(ZCOLOR_VIP13_NAME);
		}
		else if(pCharacter->IsVIP14Name()) {
			sprintf(pItem->szLevel,"%d%s",pCharacter->GetProperty()->nLevel, ZMsg(MSG_CHARINFO_LEVELMARKER));
			pItem->SetColor(ZCOLOR_VIP14_NAME);
		}	
		else if(pCharacter->IsVIP15Name()) {
			sprintf(pItem->szLevel,"%d%s",pCharacter->GetProperty()->nLevel, ZMsg(MSG_CHARINFO_LEVELMARKER));
		int i = 0;
		...
		if(i == 0)                             		// To change the colors just change the 6 numbers after 0xff to your colors (hex)
		pItem->SetColor(0xFFFF0080);			// Orange
		if(i == 1)
		pItem->SetColor(0xFF0080FF);			// Purple
		if(i == 2)
		pItem->SetColor(0xFFFF8000);			// Red	
		if(i == 3)
		pItem->SetColor(0xFFFF00FF);			// Yellow
		if(i == 4)
		pItem->SetColor(0xFF12E29D);			// Pink			
		if(i == 5)
		pItem->SetColor(0xFF48d1cc);			// Dark Blue
		if(i == 6)
		pItem->SetColor(0xFFFFFF00);			// Cyan			
		if(i == 7)
		pItem->SetColor(0xFF0000FF);			// Lime
		if(i == 8)
		pItem->SetColor(0xFF86CB2C);			// Light Blue
		if(i == 9)
		pItem->SetColor(0xFF8000FF);			// Green
		if(i == 10)
		pItem->SetColor(0xFF2EC56E);			// Pink
		if(i == 11)
		pItem->SetColor(0xFF5C1E08);			// Pink
		if(i == 12)
		pItem->SetColor(0xFFFFFFFF);			// Pink
		if(i == 13)
		pItem->SetColor(0xFF000000);			// Pink
		}			
		else
		{
		sprintf(pItem->szLevel,"%d%s",pCharacter->GetProperty()->nLevel, ZMsg(MSG_CHARINFO_LEVELMARKER));
...
// Edit khong bi mo nhan vat khi cuoi xuong
		/* if(bPick) {
...
//HP			
					...
					if(ZGetConfiguration()->GetEtc()->bfps)
					{
						pDC->SetFont(GetGameFont());
						char szMsg[70];
						...
						TextRelative(pDC, 750.f/800.f, 3.f/600.f,szMsg);
					}
