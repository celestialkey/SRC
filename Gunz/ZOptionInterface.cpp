	...
	*/
	...
	//	ComboBox
	{
		...
		if(pWidget)
		{
			...
			pWidget->SetSelIndex( iter->first );	// Fix khi thay doi do phang giai cua game
		}
	...
	{	
		...
		{
			char szBuf[64];
			sprintf(szBuf, "%i", Z_ETC_FRAMELIMIT_PERSECOND);
			pEdit->SetText(szBuf);
		...
		// Edit ShowFPS
		MButton* pfps = (MButton*)pResource->FindWidget("fps");
		if( pfps )
		{
			pfps->SetCheck(ZGetConfiguration()->GetEtc()->bfps);
		}
		...
		if( phpap )
		{
			phpap->SetCheck(ZGetConfiguration()->GetEtc()->bhpap);
		}
...
//		ZGetInput()->UnregisterActionKey(i);
	...
	*/
	...
	{	// Etc
		...
		if (pEdit)
		{
			int nPreviousFPS = Z_ETC_FRAMELIMIT_PERSECOND;
		...
		if(pfps)
		{
			Z_ETC_FPS = pfps->GetCheck();

				if (Z_ETC_FPS)
					ZGetConfiguration()->GetEtc()->bfps = true;
				else
					ZGetConfiguration()->GetEtc()->bfps = false;
		}
		...
		if(phpap)
		{
			Z_ETC_HPAP = phpap->GetCheck();

				if (Z_ETC_HPAP)
					ZGetConfiguration()->GetEtc()->bhpap = true;
				else
					ZGetConfiguration()->GetEtc()->bhpap = false;
		}
	}
