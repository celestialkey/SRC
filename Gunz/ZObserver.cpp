...
void ZObserver::OnDraw(MDrawContext* pDC)
{
		...
		if ( m_pTargetCharacter->IsAdminName())
	{
		...
		else if ( m_pTargetCharacter->IsRootName())
	{
		int i = 0;
		int nCount = ( timeGetTime() / 500)% 14; // To change follow this:  999 = time  / 10 = how many colors
		for ( i; i < nCount;  i++);
		...
		else if ( m_pTargetCharacter->IsVIP1Name())
			pDC->SetColor(MCOLOR(ZCOLOR_VIP1_NAME));
		else if ( m_pTargetCharacter->IsVIP2Name())
			pDC->SetColor(MCOLOR(ZCOLOR_VIP2_NAME));
		else if ( m_pTargetCharacter->IsVIP3Name())
			pDC->SetColor(MCOLOR(ZCOLOR_VIP3_NAME));
		else if ( m_pTargetCharacter->IsVIP4Name())
			pDC->SetColor(MCOLOR(ZCOLOR_VIP4_NAME));
		else if ( m_pTargetCharacter->IsVIP5Name())
			pDC->SetColor(MCOLOR(ZCOLOR_VIP5_NAME));
		else if ( m_pTargetCharacter->IsVIP6Name())
			pDC->SetColor(MCOLOR(ZCOLOR_VIP6_NAME));
		else if ( m_pTargetCharacter->IsVIP7Name())
			pDC->SetColor(MCOLOR(ZCOLOR_VIP7_NAME));
		else if ( m_pTargetCharacter->IsVIP8Name())
			pDC->SetColor(MCOLOR(ZCOLOR_VIP8_NAME));
		else if ( m_pTargetCharacter->IsVIP9Name())
			pDC->SetColor(MCOLOR(ZCOLOR_VIP9_NAME));
		else if ( m_pTargetCharacter->IsVIP10Name())
			pDC->SetColor(MCOLOR(ZCOLOR_VIP10_NAME));
		else if ( m_pTargetCharacter->IsVIP11Name())
			pDC->SetColor(MCOLOR(ZCOLOR_VIP11_NAME));
		else if ( m_pTargetCharacter->IsVIP12Name())
			pDC->SetColor(MCOLOR(ZCOLOR_VIP12_NAME));
		else if ( m_pTargetCharacter->IsVIP13Name())
			pDC->SetColor(MCOLOR(ZCOLOR_VIP13_NAME));
		else if ( m_pTargetCharacter->IsVIP14Name())
			pDC->SetColor(MCOLOR(ZCOLOR_VIP14_NAME));
		else if ( m_pTargetCharacter->IsVIP15Name())
	{
		int i = 0;
		int nCount = ( timeGetTime() / 500)% 14; // To change follow this:  999 = time  / 10 = how many colors
		for ( i; i < nCount;  i++);
